...
class PipePair {
  ...
  hit(birdX, birdY, birdHeight, birdWidth) {
    birdWidth /= 2;
    birdHeight /= 2;
    if((birdY-birdHeight < this.y1 + 378 * this.pipeScale) || (birdY+birdHeight > this.y2 - 378 * this.pipeScale)) {
      if(this.x < birdX-birdWidth && birdX+birdWidth < this.x + 52 * this.pipeScale) {
        return true;
      }
    }
    return false;
